...
fn main() {
    ...
    // let s = "hello, how are you?".as_bytes();
    // file.write( s).expect("writing failed");
    //let mut file = OpenOptions::new()
      //  .append(true)
        //.open("src/example.txt")
        //.expect("cannot open file");
   // file.write_all(" Adding content to file\n".as_bytes()).expect("cannot write to file");
    let mut file = File::open("src/example.txt").unwrap();
    let mut contents = String::new();
    file.read_to_string(&mut contents).unwrap();
    println!("{}", contents);

    remove_file("src/example.txt").expect("deletion failed")
}
